...
import { ScanResult, MLPrediction } from '../types';
...
export default function ScanResultView({
  ...
  const [aiResponse, setAiResponse] =
    useState<DynamicAICoachResponse | null>(null);

  const [activeModalTab, setActiveModalTab] =
    useState<'risk' | 'verify' | 'mitre'>('risk');
  ...
  const calculateHealthGrade = () => {
    ...
    if (
      ...
      return {
        ...
        color:
          'border-emerald-500/50 text-emerald-400 bg-emerald-950/30 shadow-[0_0_25px_rgba(16,185,129,0.2)]',
      };
    }
    ...
    if (score >= 85) {
      return {
        ...
        label: 'EXCELLENT',
        color:
          'border-green-500/50 text-green-400 bg-green-950/30 shadow-[0_0_25px_rgba(34,197,94,0.2)]',
      };
    }
    ...
    if (score >= 70) {
      return {
        ...
        label: 'GOOD SECURITY',
        color:
          'border-cyan-500/50 text-cyan-400 bg-cyan-950/30 shadow-[0_0_25px_rgba(6,182,212,0.2)]',
      };
    }
    ...
    if (score >= 50) {
      return {
        ...
        color:
          'border-amber-500/50 text-amber-400 bg-amber-950/30 shadow-[0_0_25px_rgba(245,158,11,0.2)]',
      };
    }
    ...
    return {
      score,
      grade: 'F',
      label: 'CRITICAL RISK (FAIL)',
      color:
        'border-rose-500/50 text-rose-400 bg-rose-950/40 shadow-[0_0_25px_rgba(244,63,94,0.25)]',
    };
  };
  ...
  const getFixSnippet = (
    vuln: Vulnerability & Record<string, any>
  ) => {
    ...
    if (badSnippet) {
      ...
      badSnippet = String(badSnippet)
        ...
        .replace(/\/\/\s*❌.*?\n/g, '')
        .replace(
          /\$\{vuln\.line_number\s*\|\|\s*1\}/g,
          String(vuln.line_number || 1)
        );

    } else {

      badSnippet =
        `No vulnerable code pattern provided for line ${
          vuln.line_number || 1
        }`;
    ...
    if (goodSnippet) {
      ...
      goodSnippet = String(goodSnippet)
        .replace(/\\n/g, '\n');
  ...
  const exportCSV = () => {
    ...
    let csvContent =
      'data:text/csv;charset=utf-8,Type,Severity,Line,Suggestion\n';

    scanResult.vulnerabilities.forEach((v) => {

      csvContent +=
        `"${v.type || v.vulnerability_type}",` +
        `"${v.severity || 'HIGH'}",` +
        `"${v.line_number || 1}",` +
        `"${(v.suggestion || '').replace(/"/g, '""')}"\n`;
  ...
  const handleAskAICoach = async (
    ...
    try {
      ...
      setAiResponse({
        ...
        why_dangerous:
          '⚠️ Backend API Server သို့ မချိတ်ဆက်နိုင်ပါ။ (localhost:8000 နှင့် GROQ_API_KEY စစ်ဆေးပါ)',

        risk_analysis:
          'AI Engine တုံ့ပြန်မှု မရရှိပါသဖြင့် Local Fallback Data ကို ပြသထားပါသည်။',

        verification_step:
          '# Unable to fetch verification command',

        recommendation:
          '// Error: Please verify backend service availability.',
      });
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      <div
        className={`border rounded-2xl p-8 shadow-2xl backdrop-blur-2xl transition-all space-y-8 ${
          themeMode === 'matrix'
            ? 'bg-zinc-950/90 border-emerald-900/80 shadow-[0_0_30px_rgba(16,185,129,0.1)]'
            : 'bg-slate-900/60 border-slate-800/90 shadow-2xl'
        }`}
      >
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 font-mono">
              ...
              <span className="text-cyan-400 font-semibold">
                {scanResult.total_issues} Findings
              </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div
            className={`lg:col-span-7 p-6 rounded-2xl border backdrop-blur-xl flex flex-col justify-between transition-all ${health.color}`}
          >
        ...
        {mlPrediction && (
          ...
          <div
            className={`relative overflow-hidden rounded-2xl border p-6 backdrop-blur-xl transition-all ${
              mlPrediction.prediction === 'VULNERABLE'
                ? 'bg-rose-950/20 border-rose-500/40 shadow-[0_0_30px_rgba(244,63,94,0.12)]'
                : mlPrediction.prediction === 'SAFE'
                ? 'bg-emerald-950/20 border-emerald-500/30 shadow-[0_0_30px_rgba(16,185,129,0.10)]'
            ...
            <div className="relative z-10">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-mono font-bold tracking-[0.2em] text-cyan-400 uppercase">
                      MACHINE LEARNING SECURITY ANALYSIS
                    </p>

                    <h3 className="text-lg font-black text-white">
                      SVM Vulnerability Classification
                    </h3>
                ...
                <span
                  className={`px-4 py-1.5 rounded-full text-[10px] font-mono font-black border ${
                    mlPrediction.prediction === 'VULNERABLE'
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-4">
                  ...
                  <p
                    className={`mt-2 text-sm font-black font-mono ${
                      mlPrediction.prediction === 'VULNERABLE'
                        ? 'text-rose-400'
                        : mlPrediction.prediction === 'SAFE'
                        ? 'text-emerald-400'
                ...
                <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-4">
                  ...
                  <p className="mt-2 text-sm font-black font-mono text-cyan-400">

                    {mlPrediction.confidence !== null &&
                    mlPrediction.confidence !== undefined
                ...
                <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-4">
                  ...
                  <p className="mt-2 text-sm font-black font-mono text-purple-400">
                    {mlPrediction.model}
                  </p>
              ...
              {mlPrediction.prediction === 'VULNERABLE' && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
                  ...
                  <div className="bg-rose-950/20 border border-rose-500/20 rounded-xl p-4">
                    ...
                    <p className="text-[10px] text-rose-400 font-bold font-mono uppercase tracking-wider mb-2">
                      Detected Vulnerability
                    </p>

                    <p className="text-sm text-slate-200 font-semibold">
                      {mlPrediction.vulnerability}
                    </p>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <span className="px-2 py-1 rounded-md bg-rose-500/10 border border-rose-500/20 text-rose-300 text-[10px] font-mono font-bold">
                        {mlPrediction.cwe}
                      </span>
                  ...
                  <div className="bg-cyan-950/20 border border-cyan-500/20 rounded-xl p-4">
                    ...
                    <p className="text-[10px] text-cyan-400 font-bold font-mono uppercase tracking-wider mb-2">
                      ML Recommendation
                    </p>
              ...
              {mlPrediction.prediction === 'SAFE' && (
                ...
                <div className="mt-4 bg-emerald-950/20 border border-emerald-500/20 rounded-xl p-4">
              ...
              {mlPrediction.prediction === 'UNAVAILABLE' && (

                <div className="mt-4 bg-amber-950/20 border border-amber-500/20 rounded-xl p-4">
              ...
              <div className="mt-4 pt-3 border-t border-slate-800/70 flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="text-[9px] font-mono text-slate-600">
                  ISVS MACHINE LEARNING ENGINE
                </span>

                <span className="text-[9px] font-mono text-slate-500">
                  Binary Classification • TF-IDF + LinearSVC
                </span>
        ...
        >
          ...
          <span>
            View 30-Day Risk & Impact Projection
          </span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
                      ...
                      <span
                        className={`text-[10px] font-mono font-extrabold px-3 py-1 rounded-full border shadow-sm ${
                          (
                            vuln.severity ||
                            'HIGH'
                          ).toUpperCase() ===
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 text-xs font-mono pt-2">
                      ...
                      <div className="bg-rose-950/20 border border-rose-500/25 rounded-xl p-4 text-rose-300 space-y-2">
                        ...
                        <pre className="whitespace-pre-wrap leading-relaxed overflow-x-auto text-[11px] font-mono text-rose-200">
                          {fix.bad}
                        </pre>
                      ...
                      <div className="bg-emerald-950/20 border border-emerald-500/25 rounded-xl p-4 text-emerald-300 space-y-2">
                        ...
                        <pre className="whitespace-pre-wrap leading-relaxed overflow-x-auto text-[11px] font-mono text-emerald-300">
                          {fix.good}
                        </pre>
                    ...
                    >
                      ...
                      <span>
                        Ask AI Security Coach for Groq Llama-3 Analysis
                      </span>
              ...
              {aiLoading ? (
                ...
                <div className="py-20 text-center space-y-4">
                  ...
                  <p className="text-sm font-bold text-white font-mono">
                    Analyzing Security Pattern with Groq LPU...
                  </p>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <div className="bg-rose-950/30 border border-rose-500/30 p-3 rounded-2xl flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-rose-400 font-bold uppercase font-mono">
                          CVSS SCORE
                        </p>

                        <p className="text-xs font-black text-white">
                          {aiResponse.cvss_severity ??
                            'HIGH'}
                        </p>
                    ...
                    <div className="bg-emerald-950/30 border border-emerald-500/30 p-3 rounded-2xl flex items-center justify-between">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-emerald-400 font-bold uppercase font-mono">
                          Status
                        </p>
                    ...
                    <div className="bg-slate-950/80 border border-amber-500/30 rounded-2xl p-4 space-y-3 font-mono">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-amber-400 font-bold uppercase">
                          🧪 Local Verification & Audit Command
                        </p>
                  ...
                  {activeModalTab ===
                    ...
                    <div className="bg-slate-950/85 border border-purple-500/30 rounded-2xl p-4 space-y-2 font-mono text-xs text-slate-300">
                      ...
                      <p className="text-purple-400 font-bold">
                        🛡️ MITRE ATT&CK Framework Context
                      </p>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between text-xs font-mono text-slate-400">
                      ...
                      <span className="text-cyan-400 font-semibold">
                        &gt; SUGGESTED REFACTORING (DIFF VIEW)
                      </span>

                      <span>
                        LANGUAGE: PYTHON / SECURE CODE
                      </span>
